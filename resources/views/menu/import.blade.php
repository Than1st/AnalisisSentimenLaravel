            ...
            <form action="{{ route('uploaddata') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <input type="file" class="form-control-file" name="fileImport" onchange="onChangeFile(this.value)"
                           @if(count($tweets)!=0) disabled @endif>
                    @error('fileImport')
                    <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <input type="submit" class="btn btn-primary btn-md btn-block" id="btnSubmit"
                       value="Import Data ke Database" onclick="loadingSwal()" disabled>
            </form>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <div class="d-flex align-items-center justify-content-between mb-1 p-1">
                    <div>
                        <button class="btn btn-danger btn-md btn-block" data-bs-toggle="modal"
                                data-bs-target="#myModal" <?php if (count($tweets) == 0) {
                            echo "disabled";
                        } ?>>Hapus Semua Data
                        </button>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Aksi ini akan menghapus semua yang berkaitan dengan data twitter, seperti data
                                        <b>Preprocessing</b>. Apakah anda yakin ingin menghapus data?
                                    </div>

                                    <!-- Modal footer -->
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal">Batal
                                        </button>
                                        <form action="{{ route('deletedata') }}" method="POST"
                                              enctype="multipart/form-data">
                                            @csrf
                                            <button type="submit" class="btn btn-danger" data-bs-dismiss="modal">Hapus
                                            </button>
                                        </form>
                                    </div>
